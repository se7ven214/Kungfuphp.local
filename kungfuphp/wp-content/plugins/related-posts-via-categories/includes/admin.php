...
$GLOBALS['relatedPostsViaCategories_admin'] = new relatedPostsViaCategories_admin();

add_action( 'admin_menu', array( 'relatedPostsViaCategories_admin', 'add' ) );


class relatedPostsViaCategories_admin
{

	function add() {
		$page = add_options_page( __('Related Posts via Categories','related-posts-via-categories'), __('Related Posts via Categories','related-posts-via-categories'), 'manage_options', 'related-posts-via-categories', array( 'relatedPostsViaCategories_admin', 'admin' ) );
		add_action( "admin_head-$page", array( 'relatedPostsViaCategories_admin', 'head' ) );
	}
	...
	function head() {
		...
		<style type="text/css">
		<!--
		#relatedPostsViaCategories-tabs {
			padding: 15px 5px 20px 5px;
			border-left: solid 1px #888888;
			border-right: solid 1px #888888;
			border-radius: 10px;
		}
		#relatedPostsViaCategories-tabs ul {
			margin: 0;
			padding: 0 5px;
			border-bottom: solid 1px #888888;
		}
		#relatedPostsViaCategories-tabs ul li {
			display: inline;
			margin: 1px;
			padding: 5px 20px;
		...
		#relatedPostsViaCategories-tabs div {
			padding: 20px 15px;
		}
		.control-checkboxes {
	...
	function admin() {
		...
		if ( $_POST['submit'] ) {
			...
			if ( $option_data != $option_post ) {
				if( !update_option( "related_posts_via_categories", $option_post ) ) {
					$message = __( "Updated Failed", 'related-posts-via-categories' );
				}else{
					$message = __( "Updated", 'related-posts-via-categories' );
					$option_data = get_option( "related_posts_via_categories" );
				}
			}
		}
		...
		jQuery( function() {
			...
			jQuery( '#relatedPostsViaCategories-tabs > ul > li' ) . click( function () {
				var str = jQuery( 'input', this ) . val();
				jQuery( '#relatedPostsViaCategories-tabs > div' ) . not( str ) . css( 'display', 'none' );
				jQuery( str ) . css( 'display', 'block' );
				jQuery( this ) . css( { 'backgroundColor': '#ffffff', 'border-bottom': 'none', 'color': '#222222' } );
				jQuery( '#relatedPostsViaCategories-tabs > ul > li' ) . not( this ) . css( { 'backgroundColor': '#e3e3e3', 'border-bottom': 'solid 1px #888888', 'color': '#21759B' } );
			} ) . first() . click();
		...
		<div class="wrap">
			<h2><?php _e( 'Related Posts via Categories - Options', 'related-posts-via-categories' ) ?></h2>
			<ul>
				<li><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=JQCM6H5BSMZSY" target="_blank"><?php _e( 'Please pay if you like this plugin.', 'related-posts-via-categories' ); ?></a></li>
				<li><?php _e( "Official page", 'related-posts-via-categories' ); ?> : <a href="http://alphasis.info/developments/wordpress-plugins/related-posts-via-categories/" target="_blank"><?php _e( "English", 'related-posts-via-categories' ); ?></a> / <a href="http://alphasis.info/kaihatu/wordpress-plugins/related-posts-via-categories/" target="_blank"><?php _e( "Japanese", 'related-posts-via-categories' ); ?></a></li>
			</ul>
			<div class="message"><p><?php echo $message; ?></p></div>

			<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?page=related-posts-via-categories">

			<div id="relatedPostsViaCategories-tabs">

				<ul>
					<li>
						<input id="#relatedPostsViaCategories-tab-1" type="hidden" value="#relatedPostsViaCategories-tab-1-contents" />
						<?php _e( 'Display','related-posts-via-categories' ) ?>
					</li>
					<li>
						<input id="#relatedPostsViaCategories-tab-2" type="hidden" value="#relatedPostsViaCategories-tab-2-contents" />
						<?php _e( 'Order','related-posts-via-categories' ) ?>
					</li>
					<li>
						<input id="#relatedPostsViaCategories-tab-3" type="hidden" value="#relatedPostsViaCategories-tab-3-contents" />
						<?php _e( 'Exclude','related-posts-via-categories' ) ?>
					</li>
					<li>
						<input id="#relatedPostsViaCategories-tab-4" type="hidden" value="#relatedPostsViaCategories-tab-4-contents" />
						<?php _e( 'Post types','related-posts-via-categories' ) ?>
					</li>
					<li>
						<input id="#relatedPostsViaCategories-tab-5" type="hidden" value="#relatedPostsViaCategories-tab-5-contents" />
						<?php _e( 'Promotion','related-posts-via-categories' ) ?>
					</li>
				</ul>

				<div id="relatedPostsViaCategories-tab-1-contents">

					<table class="form-table">
	
						<tr valign="top">
							<th scope="row"><?php _e( "Auto display", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input id="relatedPostsViaCategories-auto-display" type="checkbox" name="auto_display" value="yes" <?php if( $option_data['auto_display'] ){ echo " checked"; } ?> />
								<label for="relatedPostsViaCategories-auto-display"><?php _e( 'This option automatically displays the related posts list after the content on any single post page.', 'related-posts-via-categories' ); ?></label>
								<br /><?php _e( 'If this option is off, you will need to manually insert "display_related_posts_via_categories()" into your theme files.', 'related-posts-via-categories' ); ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "Maximum number of related posts", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="maximum_number_of_related_posts" value="<?php echo $option_data['maximum_number_of_related_posts']; ?>" size="2" />
								<?php _e('If you do not enter anything','related-posts-via-categories'); echo ': 10'; ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "Related posts title", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="related_posts_title" size="50" />
								<?php _e('For example','related-posts-via-categories'); echo ': '; _e('Related Posts'); ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "Before related posts title and related posts", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="before_related_posts_title_and_related_posts" size="50" />
								<?php _e('For example','related-posts-via-categories'); echo ': &lt;div id=&#34;related-posts-via-categories&#34;&gt;'; ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "After related posts title and related posts", 'related-posts-via-categories' ); ?>:</th>
							<td>
								...
								<?php _e('For example','related-posts-via-categories'); echo ': &lt;/div&gt;'; ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "Before related posts title", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="before_related_posts_title" size="50" />
								<?php _e('For example','related-posts-via-categories'); echo ': &lt;h2 id=&#34;related-posts-via-categories-title&#34;&gt;'; ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "After related posts title", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="after_related_posts_title" value="<?php echo $option_data['after_related_posts_title']; ?>" />
								<?php _e('For example','related-posts-via-categories'); echo ': &lt;/h2&gt;'; ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "Before related posts", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="before_related_posts" size="50" />
								<?php _e('For example','related-posts-via-categories'); echo ': &lt;ul id=&#34;related-posts-via-categories-list&#34;&gt;'; ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "After related posts", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="after_related_posts" value="<?php echo $option_data['after_related_posts']; ?>" />
								<?php _e('For example','related-posts-via-categories'); echo ': &lt;/ul&gt;'; ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "Before each related post", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="before_each_related_post" size="50" />
								<?php _e('For example','related-posts-via-categories'); echo ': &lt;li class=&#34;related-posts-via-categories-item&#34;&gt;'; ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "After each related post", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="after_each_related_post" value="<?php echo $option_data['after_each_related_post']; ?>" />
								<?php _e('For example','related-posts-via-categories'); echo ': &lt;/li&gt;'; ?>
							</td>
						</tr>
				...
				<div id="relatedPostsViaCategories-tab-2-contents">

					<table class="form-table">
	
						<tr valign="top">
							<th scope="row"><?php _e('Order By', 'related-posts-via-categories'); ?>:</th>
							<td>
								<input id="related_scores_high__speedy" name="order_by" type="radio" value="related_scores_high__speedy"<?php if($option_data['order_by']=="related_scores_high__speedy")echo ' checked'; ?> /><label for="related_scores_high__speedy"><?php _e(' Related Scores : High', 'related-posts-via-categories'); ?>&nbsp;&nbsp;(&nbsp;<?php _e('Speedy', 'related-posts-via-categories'); ?>&nbsp;)</label>
								<br /><input id="related_scores_high__date_published_new" name="order_by" type="radio" value="related_scores_high__date_published_new"<?php if($option_data['order_by']=="related_scores_high__date_published_new")echo ' checked'; ?> /><label for="related_scores_high__date_published_new"><?php _e(' Related Scores : High', 'related-posts-via-categories'); ?>&nbsp;&nbsp;/&nbsp;<?php _e(' Date Published : New', 'related-posts-via-categories'); ?>&nbsp;&nbsp;(&nbsp;<?php _e('Default setting', 'related-posts-via-categories'); ?>&nbsp;)</label>
								<br /><input id="related_scores_high__date_published_old" name="order_by" type="radio" value="related_scores_high__date_published_old"<?php if($option_data['order_by']=="related_scores_high__date_published_old")echo ' checked'; ?> /><label for="related_scores_high__date_published_old"><?php _e(' Related Scores : High', 'related-posts-via-categories'); ?>&nbsp;&nbsp;/&nbsp;<?php _e(' Date Published : Old', 'related-posts-via-categories'); ?></label>
								<br /><input id="related_scores_low__date_published_new" name="order_by" type="radio" value="related_scores_low__date_published_new"<?php if($option_data['order_by']=="related_scores_low__date_published_new")echo ' checked'; ?> /><label for="related_scores_low__date_published_new"><?php _e(' Related Scores : Low', 'related-posts-via-categories'); ?>&nbsp;&nbsp;/&nbsp;<?php _e(' Date Published : New', 'related-posts-via-categories'); ?></label>
								<br /><input id="related_scores_low__date_published_old" name="order_by" type="radio" value="related_scores_low__date_published_old"<?php if($option_data['order_by']=="related_scores_low__date_published_old")echo ' checked'; ?> /><label for="related_scores_low__date_published_old"><?php _e(' Related Scores : Low', 'related-posts-via-categories'); ?>&nbsp;&nbsp;/&nbsp;<?php _e(' Date Published : Old', 'related-posts-via-categories'); ?></label>
								<br /><input id="related_scores_high__date_modified_new" name="order_by" type="radio" value="related_scores_high__date_modified_new"<?php if($option_data['order_by']=="related_scores_high__date_modified_new")echo ' checked'; ?> /><label for="related_scores_high__date_modified_new"><?php _e(' Related Scores : High', 'related-posts-via-categories'); ?>&nbsp;&nbsp;/&nbsp;<?php _e(' Date Modified : New', 'related-posts-via-categories'); ?></label>
								<br /><input id="related_scores_high__date_modified_old" name="order_by" type="radio" value="related_scores_high__date_modified_old"<?php if($option_data['order_by']=="related_scores_high__date_modified_old")echo ' checked'; ?> /><label for="related_scores_high__date_modified_old"><?php _e(' Related Scores : High', 'related-posts-via-categories'); ?>&nbsp;&nbsp;/&nbsp;<?php _e(' Date Modified : Old', 'related-posts-via-categories'); ?></label>
								<br /><input id="related_scores_low__date_modified_new" name="order_by" type="radio" value="related_scores_low__date_modified_new"<?php if($option_data['order_by']=="related_scores_low__date_modified_new")echo ' checked'; ?> /><label for="related_scores_low__date_modified_new"><?php _e(' Related Scores : Low', 'related-posts-via-categories'); ?>&nbsp;&nbsp;/&nbsp;<?php _e(' Date Modified : New', 'related-posts-via-categories'); ?></label>
								<br /><input id="related_scores_low__date_modified_old" name="order_by" type="radio" value="related_scores_low__date_modified_old"<?php if($option_data['order_by']=="related_scores_low__date_modified_old")echo ' checked'; ?> /><label for="related_scores_low__date_modified_old"><?php _e(' Related Scores : Low', 'related-posts-via-categories'); ?>&nbsp;&nbsp;/&nbsp;<?php _e(' Date Modified : Old', 'related-posts-via-categories'); ?></label>
								<br /><input id="date_published_new" name="order_by" type="radio" value="date_published_new"<?php if($option_data['order_by']=="date_published_new")echo ' checked'; ?> /><label for="date_published_new"><?php _e(' Date Published : New', 'related-posts-via-categories'); ?></label>
								<br /><input id="date_published_old" name="order_by" type="radio" value="date_published_old"<?php if($option_data['order_by']=="date_published_old")echo ' checked'; ?> /><label for="date_published_old"><?php _e(' Date Published : Old', 'related-posts-via-categories'); ?></label>
								<br /><input id="date_modified_new" name="order_by" type="radio" value="date_modified_new"<?php if($option_data['order_by']=="date_modified_new")echo ' checked'; ?> /><label for="date_modified_new"><?php _e(' Date Modified : New', 'related-posts-via-categories'); ?></label>
								<br /><input id="date_modified_old" name="order_by" type="radio" value="date_modified_old"<?php if($option_data['order_by']=="date_modified_old")echo ' checked'; ?> /><label for="date_modified_old"><?php _e(' Date Modified : Old', 'related-posts-via-categories'); ?></label>
							</td>
						</tr>
				...
				<div id="relatedPostsViaCategories-tab-3-contents">

					<table class="form-table">

						<tr valign="top">
							<th scope="row"><?php _e( "Exclude (Posts)", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input type="text" name="exclude_posts" value="<?php echo $option_data['exclude_posts']; ?>" size="50" />
								<?php _e('The IDs of any posts you want to exclude, separated by commas.','related-posts-via-categories'); ?>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row">
								<?php _e( "Exclude (Categories)", 'related-posts-via-categories' ); ?>:
		...
		<?php endif; ?>
				...
				<div id="relatedPostsViaCategories-tab-4-contents">

					<table class="form-table">

						<tr valign="top">
							<th scope="row"><?php _e( "Post types", 'related-posts-via-categories' ); ?>:</th>
							<td>
								...
								<label for="post_types_post"><?php _e( 'post', 'related-posts-via-categories' ); ?></label>
		...
		<?php endif; ?>
				...
				<div id="relatedPostsViaCategories-tab-5-contents">

					<table class="form-table">
	
						<tr valign="top">
							<th scope="row"><?php _e( "Could you promote this plugin?", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input id="promotion_link" type="checkbox" name="promotion_link" value="yes" <?php if ( $option_data['promotion_link'] ) { echo 'checked '; }?>/>
								<label for="promotion_link"><?php _e( 'If this option is enabled, displays the link to the official page of this plugin after the related posts list.', 'related-posts-via-categories' ); ?></label>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row"><?php _e( "Link text", 'related-posts-via-categories' ); ?>:</th>
							<td>
								<input id="promotion_link_text_1" type="radio" name="promotion_link_text" value="To the official site of Related Posts via Categories." <?php if ( $option_data['promotion_link_text'] == 'To the official site of Related Posts via Categories.' ) { echo 'checked '; }?>/>
								<label for="promotion_link_text_1">To the official site of Related Posts via Categories.</label>
								<br />
								<input id="promotion_link_text_2" type="radio" name="promotion_link_text" value="Related Posts via Categories" <?php if ( $option_data['promotion_link_text'] == 'Related Posts via Categories' ) { echo 'checked '; }?>/>
								<label for="promotion_link_text_2">Related Posts via Categories</label>
								<br />
						...
						<tr valign="top">
							<th scope="row"><?php _e( "Language of the official page", 'related-posts-via-categories' ); ?>:</th>
							<td>
								...
								<label for="promotion_link_language_english" style="margin-right: 15px;"><?php _e( "English", 'related-posts-via-categories' ); ?></label>
								...
								<label for="promotion_link_language_japanese" style="margin-right: 15px;"><?php _e( "Japanese", 'related-posts-via-categories' ); ?></label>
							</td>
						</tr>
				...
				<p class="submit">
				<input type="submit" value="<?php _e('Save changes','related-posts-via-categories') ?>" name="submit" />
				</p>
