...
$GLOBALS['relatedPostsViaCategories_display'] = new relatedPostsViaCategories_display();


class relatedPostsViaCategories_display
{
	...
	function core() {
		...
		$exclude_term_taxonomy_ids = $wpdb->get_col( "SELECT term_taxonomy_id FROM {$wpdb->term_taxonomy} WHERE taxonomy = 'post_tag'" );
		...
		if ( $custom_taxonomies ) {
			foreach ( $custom_taxonomies as $custom_taxonomy ) {
				$exclude_term_taxonomy_ids = $wpdb->get_col( "SELECT term_taxonomy_id FROM {$wpdb->term_taxonomy} WHERE taxonomy = '{$custom_taxonomy}'" );
				if ( $exclude_term_taxonomy_ids ) {
					$exclude_term_taxonomy_ids_str = implode( ",", $exclude_term_taxonomy_ids );
					$args .= " AND term_taxonomy_id NOT IN ( {$exclude_term_taxonomy_ids_str} )";
				}
		...
		if ( $option_data['promotion_link'] ) {
			...
			if ( $option_data['promotion_link_language'] == 'japanese' ) {
				$output .= '<a href="http://alphasis.info/kaihatu/wordpress-plugins/related-posts-via-categories/" title="To the official site of Related Posts via Categories." target="_blank">' . $option_data['promotion_link_text'] . '</a>';
			} else {
				$output .= '<a href="http://alphasis.info/developments/wordpress-plugins/related-posts-via-categories/" title="To the official site of Related Posts via Categories." target="_blank">' . $option_data['promotion_link_text'] . '</a>';
			}
			$output .= '</p>';
		}
		$output .= $wrap_after;
